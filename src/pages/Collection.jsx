...
const Collection = () => {
  ...
  return (
    <div className='flex flex-col sm:flex-row gap-1 sm:gap-10 pt-10 border-t' >
      ...
      {/* Filter Options */}
      <div className='min-w-60 posi'>
        <p onClick={()=>setShowFilter(!showFilter)} className='my-2 text-xl flex items-center cursor-pointer gap-2'>Filter
          <img className={`h-3 sm:hidden ${showFilter ? "rotate-90" : ""}`} src={assets.dropdown_icon} alt="" />
        </p>
        {/* Category Filter */}
        <div className={`rounded-lg bg-gray-600 border border-gray-300 pl-5 py-3 mt-6 ${showFilter ? "" : "hidden"} sm:block `}>
        ...
        {/* SUb-Category Filter */}
        <div className={`rounded-lg border bg-gray-600 border-gray-300 pl-5 py-3 my-5 ${showFilter ? "" : "hidden"} sm:block `}>
        ...
        <div className='grid gird-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 gap-y-6'>
          {
            filterProducts.map((item, index)=>{
             return <SingleProduct key={index} name={item.name} id={item._id} price={item.price} image={item.image} />
            })
          }
        </div>
      </div>
